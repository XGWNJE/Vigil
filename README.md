...
**UI / UX**
- 全套深色主题 UI（背景 #0A0A0A，卡片 #141414，边框 #27272A），紫色主色调 #A855F7
- 监控页全新布局：状态卡片含图标+状态徽章+标题+副标题+可展开诊断信息，服务开关改为卡片式，权限状态独立绿色提示卡，关键词区域含数量徽章
- 关键词输入改为 Chip 标签式，支持独立添加和删除，操作即保存
- 应用过滤从 Settings 页内嵌 400dp 嵌套 LazyColumn，改为独立全屏 `AppFilterScreen`，消除嵌套滚动冲突
- Settings 页权限列表：图标+文字+右侧状态（✓或"去设置"），支持永久去设置跳转
