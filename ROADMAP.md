...
> 2026-07-30 owner 规划会结论。当前版本 v1.16.0 (versionCode 22)。
...
每个关键词可绑定独立铃声与循环播放次数，多监听类型时用户凭铃声分辨；次数范围支持自动结束，避免无人值守时无限响铃。循环次数设置于 v1.16.0 调整为 1–10 次线性范围。
...
- 分别设 1 次与 10 次触发 → 验证自动停止及手动确认停止、弹窗关闭、pending 清除、记录已写入。
